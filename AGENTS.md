...
├── modbusapi.{h,cpp}             Modbus TCP client — async polling, register reads/writes,
│                                 connection management, request queuing
...
- **ModbusApi** must be mockable: define `IModbusApi` interface (or use virtual methods) so tests can inject `MockModbusApi` that emits signals with fake register data
- **Data structs** (`PvData`, `GridData`, etc.) are plain value types — no QObject dependency — making them trivially testable
- **Register parsing helpers** (scale, sign conversion, string parsing) must be `static` pure functions in a `RegisterParser` utility class — unit-testable without network
