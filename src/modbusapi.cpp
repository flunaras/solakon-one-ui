...
void ModbusApi::onPollTimerFired()
{
    ...
    // Read (and emitted) *before* pollBlock3/batteryDataUpdated: PvChartWidget
    // latches remoteControlStateUpdated and only bakes it into a recorded
    // sample once batteryDataUpdated fires (the last signal of the poll
    // cycle) — if REMOTE_CONTROL were read after block 3, that latch would
    // still hold the *previous* cycle's value when the sample for the
    // current cycle is recorded, making the remote-control overlay in the PV
    // chart consistently lag one sample behind the actual state change.
    enqueueRead(kAddrRemoteControl, 1, [this](const QModbusDataUnit &u) {
        parseAndEmitRemoteControl(u);
    });
