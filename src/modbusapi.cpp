...
static constexpr int kOff1Status            = 0;   // 39063
...
void ModbusApi::processNextRequest()
{
    ...
    if (auto *r = std::get_if<PendingRead>(&req)) {
        ...
        if (reply->isFinished()) {
            if (reply->error() == QModbusDevice::NoError) emit writeSucceeded(pending.name);
            else emit writeFailed(pending.name, reply->errorString());
            reply->deleteLater();
            m_requestInFlight = false;
            processNextRequest();
            return;
        ...
        connect(reply, &QModbusReply::finished, this,
                [this, reply, n = std::move(pending.name)]() {
                    if (reply->error() == QModbusDevice::NoError) emit writeSucceeded(n);
                    else emit writeFailed(n, reply->errorString());
                    reply->deleteLater();
                    m_requestInFlight = false;
                    processNextRequest();
                });
...
void ModbusApi::writeWorkMode(int modeValue) {
    sendWriteSingleRegister(kAddrWorkMode, static_cast<quint16>(modeValue),
