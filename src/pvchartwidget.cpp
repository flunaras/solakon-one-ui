...
// Remote control active overlay colour (blue-violet, low alpha so PV/grid
// data underneath stays legible; drawn as a full-height band, not tied to
// the Y axis like the power series). Kept light enough that the custom
// gridlines (drawn with the axis's own native, fairly pale line color — see
// updateGridOverlay()) still read clearly through the tint.
static const QColor kRemoteControlBandColor(0x66, 0x33, 0xCC, 25);
...
// ── X-axis (time) tick helpers ──────────────────────────────────────────────────

// Candidate step durations (ms) for the custom time gridlines, chosen so
// ticks always land on whole seconds/minutes/hours instead of arbitrary
// fractions (e.g. a naive "evenly divide the window into N ticks" approach
// gives 1.5-minute steps for a 15-minute window, which round inconsistently
// when formatted as "hh:mm" — alternating +1/+2 minute label jumps even
// though the underlying gridlines are pixel-perfect evenly spaced).
static const QList<qint64> kTimeStepCandidatesMs = {
    10LL  * 1000,             // 10 s
    15LL  * 1000,             // 15 s
    30LL  * 1000,             // 30 s
    1LL   * 60 * 1000,        // 1 min
...
// Returns the smallest candidate step (from kTimeStepCandidatesMs) that keeps
// the number of ticks across rangeMs at or below maxTicks — i.e. the
// highest-resolution round step that still respects the tick budget. Falls
// back to the largest candidate if even that yields too many ticks (only
// possible for windows far beyond the app's largest supported 24 h window).
static qint64 computeNiceTimeStep(qint64 rangeMs, int maxTicks)
{
    if (rangeMs <= 0 || maxTicks < 1)
        return kTimeStepCandidatesMs.first();
    for (qint64 step : kTimeStepCandidatesMs) {
        if (rangeMs / step <= maxTicks)
            return step;
    }
    return kTimeStepCandidatesMs.last();
}
    ...
    : QWidget(parent)
{
    ...
    auto *remoteLegendSwatch = new QLabel(this);
    remoteLegendSwatch->setFixedSize(12, 12);
    // Pre-blend kRemoteControlBandColor's low alpha over white here, rather
    // than passing the raw rgba() through with alpha forced to 255: the
    // stylesheet background-color is painted on the widget's own opaque
    // background, so a literal rgba(...) with the real (low) alpha would
    // render solid at that alpha over whatever the platform theme's window
    // color happens to be, while the actual overlay band is always alpha-
    // blended over the (typically white) chart plot area — using the same
    // final blended color here keeps the swatch a faithful preview instead
    // of an oversaturated solid chip.
    const QColor blended = [] {
        const qreal a = kRemoteControlBandColor.alphaF();
        return QColor::fromRgbF(
            kRemoteControlBandColor.redF()   * a + 1.0 * (1.0 - a),
            kRemoteControlBandColor.greenF() * a + 1.0 * (1.0 - a),
            kRemoteControlBandColor.blueF()  * a + 1.0 * (1.0 - a));
    }();
    remoteLegendSwatch->setStyleSheet(
        QStringLiteral("background-color: rgb(%1,%2,%3); border: 1px solid palette(mid);")
            .arg(blended.red())
            .arg(blended.green())
            .arg(blended.blue()));
    topRow->addWidget(remoteLegendSwatch);
    topRow->addWidget(new QLabel(QStringLiteral("Remote control active"), this));
    topRow->addStretch();
...
void PvChartWidget::onRemoteControlStateUpdated(const RemoteControlState &state)
{
    // Latch only; the next sample (recorded in onBatteryDataUpdated()) picks
    // this up so remote-control spans stay aligned with the same timestamps
    // as the PV/grid/battery data instead of drifting independently.
    m_lastRemoteControlActive = state.enabled;
}
...
void PvChartWidget::resizeEvent(QResizeEvent *event)
{
    QWidget::resizeEvent(event);
    // The chart's plotArea() (and therefore every mapToPosition() result
    // used by the custom gridlines and remote-control bands) depends on the
    // widget's size, so both must be repositioned on resize — not just on
    // new data — otherwise they'd retain stale coordinates from the last
    // data-driven rebuild until the next sample arrives.
    if (!m_history.isEmpty()) {
        updateRemoteControlOverlay();
        updateGridOverlay();
        updateBatteryLineOverlay();
    }
}
...
void PvChartWidget::buildChart()
{
    ...
    // Native gridlines are always painted *below* the data series by Qt
    // Charts (no public API to reorder that), so they get hidden behind the
    // PV/grid/load/battery series and the remote-control overlay bands.
    // Disable them here and draw our own instead (see m_gridLines /
    // updateGridOverlay()) at a Z-value above everything so the grid is
    // always visible on top, exactly like the hover crosshair.
    m_axisX->setGridLineVisible(false);
    m_axisX->setMinorGridLineVisible(false);
    // Native labels are also disabled: QDateTimeAxis always divides
    // min()..max() into exactly tickCount() *evenly spaced* ticks with no
    // "nice round time" snapping, so its labels alternate irregularly
    // (e.g. 19:06, 19:08, 19:09, 19:11 — jumping +2/+1/+2 min) whenever the
    // window duration isn't a whole multiple of tickCount()-1. Custom labels
    // drawn at the same "nice" round timestamps as m_gridLines
    // (updateGridOverlay()) replace them so gridlines and labels always
    // agree and land on whole seconds/minutes/hours.
    m_axisX->setLabelsVisible(false);
    // The axis "line" (baseline + its own small perpendicular tick marks at
    // the old evenly-spaced tickCount() positions) is a separate visual
    // element from gridlines/labels and stays visible even with both of
    // those disabled — leaving stray, non-round-aligned tick marks behind
    // that don't match our custom nice-step grid. Hide it entirely.
    m_axisX->setLineVisible(false);
    m_chart->addAxis(m_axisX, Qt::AlignBottom);

    // Reserve enough right-hand margin for half a time label's width — the
    // rightmost custom gridline/label (updateGridOverlay()) sits at
    // plotArea's right edge (live data always extends to "now"), and its
    // label is centered on that tick, so without extra margin its right
    // half would be clipped by (or overlap) the widget/window border as new
    // ticks keep appearing at the live edge.
    {
        const QFontMetrics fm(m_axisX->labelsFont());
        // "00:00:00" (the widest label format used — see updateGridOverlay()'s
        // sub-minute "hh:mm:ss" fallback) so the reserved margin still fits
        // even when seconds are shown for short (e.g. 5-minute) windows.
        const int labelHalfWidth = fm.horizontalAdvance(QStringLiteral("00:00:00")) / 2 + 4;
        QMargins margins = m_chart->margins();
        margins.setRight(std::max(margins.right(), labelHalfWidth));
    ...
    m_axisY->setGridLineVisible(false);   // see comment on m_axisX above
    m_axisY->setMinorGridLineVisible(false);
    // Y labels stay native: QValueAxis's TicksDynamic + tickAnchor(0) already
    // guarantees round, evenly-stepped values (updateYAxis() computes a
    // {1,2,5}×10ⁿ step), so there's no equivalent rounding-artifact to fix.
    ...
    // ── Battery line overlay ─────────────────────────────────────────────────
    // Plain visual duplicate of m_netSeries, parented to m_chart so its
    // coordinates match plotArea()/mapToPosition() directly. See the m_
    // batteryLineOverlay comment in the header for why this duplicate exists
    // (native series can't be drawn above the custom-drawn gridlines).
    // Invisible clip container — see the m_batteryLineClipRect comment in the
    // header for why the overlay is parented here instead of directly to
    // m_chart (prevents the trace from bleeding outside the plot area when
    // an off-range anchor point is included in m_lastBatteryPts).
    m_batteryLineClipRect = new QGraphicsRectItem(m_chart);
    m_batteryLineClipRect->setPen(Qt::NoPen);
    m_batteryLineClipRect->setBrush(Qt::NoBrush);
    m_batteryLineClipRect->setFlag(QGraphicsItem::ItemClipsChildrenToShape, true);
    m_batteryLineClipRect->setZValue(11);  // above m_gridLines (z=10)

    m_batteryLineOverlay = new QGraphicsPathItem(m_batteryLineClipRect);
    {
        QPen batteryPen(QColor(0, 0, 0));
        batteryPen.setWidth(2);
        m_batteryLineOverlay->setPen(batteryPen);
    }
    ...
    // ── Keep the custom gridlines/remote-control bands pixel-accurate ───────
    // QChart::plotAreaChanged fires whenever Qt Charts actually finalizes its
    // internal layout (axis label widths, legend height, etc. all settle
    // asynchronously after resizes/show()). Recomputing on this signal —
    // rather than relying solely on the data-driven rebuildSeries() call or
    // our own resizeEvent() override — fixes a transient one-frame
    // misalignment: right after the chart view transitions from hidden to
    // visible (see onBatteryDataUpdated()), Qt Charts' layout for that first
    // visible frame isn't final yet at the point rebuildSeries() computes
    // mapToPosition() coordinates, so the very first custom-drawn grid could
    // land a few pixels off from the axis tick labels until this signal
    // corrects it.
...
void PvChartWidget::appendSample(const PvData &data)
{
    ...
    s.remoteControlActive = m_lastRemoteControlActive;
    m_history.append(s);
    pruneHistory();
}
...
void PvChartWidget::rebuildSeries()
{
    ...
    // Cache for updateBatteryLineOverlay(), which re-maps these to scene
    // coordinates both here and on plain view resizes.
    m_lastBatteryPts = batteryPts;
...
void PvChartWidget::updateYAxis(double maxW, double minW, const QString &unitLabel)
{
    ...
    // ── 1. Raw bounds — clamp to the sign each side is expected to have (PV+
    // import stack ≥ 0, export+load+battery ≤ 0) and use the true data
    // extents directly (no coarse magnitude-based pre-rounding, which used
    // to waste a lot of vertical space, e.g. rounding a −140 W minimum up to
    // −200 W before the step size was even known).
    //
    // Note: this function is only ever called from rebuildSeries(), which
    // itself only runs once m_history is non-empty, so there's no genuine
    // "no data yet" case to special-case here — maxW/minW always reflect a
    // real (if possibly all-zero, e.g. nighttime with no PV/grid/battery
    // flow) sample. A previous version forced rawMax to a 100 W placeholder
    // whenever maxW ≤ 1 W, which incorrectly padded the axis up to +100 W
    // even when the true PV/grid stack was genuinely at 0 W.
    double rawMax = std::max(maxW, 0.0);
    double rawMin = std::min(minW, 0.0);
    // If one side is genuinely at 0 (e.g. no PV production at all, or no
    // grid/load/battery flow), give it a small +/-10 W of headroom instead
    // of leaving that side flush at the 0 line — looks better than a
    // one-sided chart pinned exactly to the axis edge, and also covers the
    // degenerate all-zero case (rawMax=10, rawMin=-10 -> a sane 20 W range)
    ...
    // ── 2. Nice tick step targeting ~6 ticks across the true range ──────────
    // computeNiceStep returns a member of {1,2,5}×10ⁿ. A target of 9 (used
    // previously) tends to land awkwardly between the {2,5}×10ⁿ tiers for
    // common power ranges (e.g. a 250 W range rounds down to a 20 W step,
    // yielding ~13 gridlines instead of ~9) — especially noticeable now that
    // the grid is drawn prominently on top of the data. 6 produces a more
    // legible, less cluttered result while still round-stepped.
    const double step = computeNiceStep(rawMax - rawMin, 6);

    // ── 3. Snap bounds outward to step multiples so axis limits land on
    // ticks, using the true (unpadded) data extents as the starting point.
    double niceMax = std::ceil (rawMax / step) * step;
    double niceMin = std::floor(rawMin / step) * step;
...
void PvChartWidget::updateRemoteControlOverlay()
{
    // Discard the previous rebuild's bands — the number/extent of
    // contiguous "active" runs varies each redraw (data changes, window
    // scrolls, etc.) so it's simplest to rebuild from scratch rather than
    // try to reuse/resize existing items.
    qDeleteAll(m_remoteControlBands);
    m_remoteControlBands.clear();
    ...
    // Walk the history and find contiguous runs of
    // remoteControlActive==true, clipped to [startMs, endMs]. A run's true
    // end is the next sample's timestamp (or endMs if it's the last sample
    // in the visible window / history), so the band always extends up to
    // the point where the state is known to have changed (or "now" for a
    // run still active at the right edge).
    qint64 runStartMs = -1;
    for (int i = 0; i < m_history.size(); ++i) {
        const Sample &sample = m_history[i];
        const qint64 sampleEndMs =
            (i + 1 < m_history.size()) ? m_history[i + 1].timestampMs : endMs;

        if (sample.remoteControlActive) {
            if (runStartMs < 0)
                runStartMs = sample.timestampMs;
        } else if (runStartMs >= 0) {
            const qint64 bandStart = std::max(runStartMs, startMs);
            const qint64 bandEnd   = std::min(sample.timestampMs, endMs);
            if (bandEnd > bandStart) {
                auto *rect = new QGraphicsRectItem(m_chart);
                rect->setBrush(kRemoteControlBandColor);
                rect->setPen(Qt::NoPen);
                rect->setZValue(-0.5);  // above QChart::background (z=-1), below series (z>=0); custom gridlines (updateGridOverlay(), z=10) sit above everything
                const qreal x1 = m_chart->mapToPosition(QPointF(bandStart, 0), m_netSeries).x();
                const qreal x2 = m_chart->mapToPosition(QPointF(bandEnd, 0), m_netSeries).x();
                rect->setRect(x1, plotArea.top(), x2 - x1, plotArea.height());
                m_remoteControlBands.append(rect);
            }
            runStartMs = -1;
        }

        if (sampleEndMs > endMs)
            break;
    }

    // Still-open run at the right edge of the visible window (remote
    // control active for the last sample seen, or the whole window).
    if (runStartMs >= 0) {
        const qint64 bandStart = std::max(runStartMs, startMs);
        const qint64 bandEnd   = endMs;
        if (bandEnd > bandStart) {
            auto *rect = new QGraphicsRectItem(m_chart);
            rect->setBrush(kRemoteControlBandColor);
            rect->setPen(Qt::NoPen);
...
void PvChartWidget::updateGridOverlay()
{
    // Rebuilt from scratch each call — same rationale as
    // updateRemoteControlOverlay(): the number of ticks/their positions can
    // change between redraws (axis range/step change, view resize), so it's
    // simplest to discard and recreate rather than reuse/reposition.
    qDeleteAll(m_gridLines);
    m_gridLines.clear();
    qDeleteAll(m_gridLabelsX);
    m_gridLabelsX.clear();
    ...
    // A clearly-visible, medium-light gray. The previous attempts (matching
    // the axis's own native line pen at (214,214,214), then a paler
    // (230,230,230)) both turned out to render as an essentially invisible
    // near-white hairline against the white plot background — QGraphicsLineItem
    // defaults to a cosmetic (0-width) pen, which some renderers draw as a
    // sub-pixel line that all but disappears at light colors. Use a darker,
    // unambiguous gray with an explicit 1px width so the grid is reliably
    // visible without overpowering the data series drawn on top of it.
    QPen pen(QColor(0xBF, 0xBF, 0xBF));
    pen.setWidth(1);
    pen.setCosmetic(true);

    // Small protrusion below the plot area for the vertical (time) ticks,
    // mirroring the look of the Y axis's own native tick marks that already
    // stick out a few pixels to the left of the plot area (m_axisY's line is
    // still enabled; only m_axisX's was hidden, since its old evenly-spaced
    // tick positions no longer match our nice-round-time positions).
    constexpr qreal kTickProtrusion = 6.0;

    // ── Vertical lines + labels at "nice" round time ticks ──────────────────
    // Rather than naively dividing min()..max() into m_axisX->tickCount()
    // evenly spaced steps (which can land on fractional-minute intervals —
    // e.g. a 15-minute window / 10 gaps = 1.5 min/gap — causing labels to
    // round inconsistently, alternating +1/+2 minute jumps), pick the
    // largest-resolution *round* step (whole seconds/minutes/hours) that
    // still keeps the tick count reasonable, then anchor ticks to absolute
    // multiples of that step so they always land on whole clock values.
    const qint64 minMs = m_axisX->min().toMSecsSinceEpoch();
    const qint64 maxMs = m_axisX->max().toMSecsSinceEpoch();
    const qint64 step  = computeNiceTimeStep(maxMs - minMs, 16);
    const qint64 firstTickMs = ((minMs + step - 1) / step) * step;  // ceil to step multiple

    const QFont labelFont   = m_axisX->labelsFont();
    const QColor labelColor = m_axisX->labelsColor();
    // Sub-minute steps (e.g. the 5-minute window's 30 s step) need seconds
    // in the label, or every tick within the same minute would render an
    // identical "hh:mm" text (e.g. two ticks both showing "21:20") — very
    // confusing since it looks like duplicated/misaligned labels rather than
    // distinct ticks 30 s apart.
    const QString labelFormat = (step < 60 * 1000) ? QStringLiteral("hh:mm:ss")
                                                     : m_axisX->format();

    for (qint64 ms = firstTickMs; ms <= maxMs; ms += step) {
        const qreal x = m_chart->mapToPosition(QPointF(static_cast<double>(ms), 0.0),
                                                m_netSeries).x();

        auto *line = new QGraphicsLineItem(x, plotArea.top(), x, plotArea.bottom() + kTickProtrusion,
                                            m_chart);
        line->setPen(pen);
        line->setZValue(10);  // above every data series and the remote-control bands
        m_gridLines.append(line);

        auto *label = new QGraphicsSimpleTextItem(m_chart);
        ...
        m_gridLabelsX.append(label);
    }

    // ── Horizontal lines at each Y-axis (power) tick ────────────────────────
    // updateYAxis() anchors ticks at 0 with a fixed step (m_axisY->tickInterval()),
    // so every tick is minY + k*step for some integer k — walk from the
    // first in-range tick up to maxY.
    const double minY = m_axisY->min();
    const double maxY = m_axisY->max();
    const double yStep = m_axisY->tickInterval();
    if (yStep > 0.0) {
        const double firstTick = std::ceil(minY / yStep) * yStep;
        for (double y = firstTick; y <= maxY + yStep * 1e-6; y += yStep) {
            const qreal py = m_chart->mapToPosition(QPointF(0.0, y), m_netSeries).y();
            auto *line = new QGraphicsLineItem(plotArea.left(), py, plotArea.right(), py, m_chart);
            line->setPen(pen);
            line->setZValue(10);
            m_gridLines.append(line);
        }
...
void PvChartWidget::updateBatteryLineOverlay()
{
    // Keep the clip rect matched to the current plot area on every redraw —
    // it can change independently of new data (resize, legend/axis layout
    // settling — see the QChart::plotAreaChanged comment in buildChart()).
    m_batteryLineClipRect->setRect(m_chart->plotArea());

    if (m_lastBatteryPts.isEmpty()) {
        m_batteryLineOverlay->setPath(QPainterPath());
        return;
    }

    // Re-maps the same points already pushed into m_netSeries to scene
    // coordinates via the same mapToPosition() used by the gridlines/bands,
    // so this duplicate trace lines up pixel-for-pixel with the native
    // series underneath it.
    QPainterPath path;
    bool first = true;
    for (const QPointF &pt : m_lastBatteryPts) {
        const QPointF scenePos = m_chart->mapToPosition(pt, m_netSeries);
        if (first) {
    ...
    m_batteryLineOverlay->setPath(path);
}
