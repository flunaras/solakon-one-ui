...
// ── Grid status (register 39065 bitfield16) ───────────────────────────────────
// Documented bits (FoxESS H3 PRO Modbus protocol V1.05):
//   bit 0 = Off-Grid / EPS    (1 = inverter in island/backup mode, 0 = grid-tied)
// Other bits are reserved; the raw value is preserved for diagnostic display.
struct GridStatus {
    bool offGrid = false;  // bit 0 — EPS / island mode
    quint16 raw  = 0;
};

// ── Remote control read-back state (register 46001 bitfield16) ────────────────
// Bit layout matches the write side (see RemoteControlWidget::buildBitfield):
//   bit 0     = enable     (1 = remote control currently active)
//   bit 1     = direction  (0 = generate/inject, 1 = consume/absorb)
//   bits 3-2  = target     (00 = AC, 01 = Battery, 10 = Grid)
// Read-back is essential to detect ownership conflicts: the FoxESS app's
// "strategy periods" feature uses the same register, so the UI must be able
// to show whether remote control is engaged externally before any local write.
struct RemoteControlState {
    bool enabled   = false;  // bit 0
    bool consume   = false;  // bit 1 (false = generate/inject, true = consume/absorb)
    int  target    = 0;      // bits 3-2 (0=AC, 1=Battery, 2=Grid, 3=reserved)
    quint16 raw    = 0;

    QString targetLabel() const {
        switch (target) {
        case 0: return QStringLiteral("AC");
        case 1: return QStringLiteral("Battery");
        case 2: return QStringLiteral("Grid");
        default: return QStringLiteral("Reserved (%1)").arg(target);
        }
    }
};
