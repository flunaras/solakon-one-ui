    ...
    : QWidget(parent)
{
    ...
    m_gridStatusLine = new QLabel(QStringLiteral("--"), this);
    m_gridStatusLine->setAlignment(Qt::AlignCenter);
    m_gridStatusLine->setStyleSheet(QStringLiteral("font-weight: bold;"));
    ...
    form->addRow(QStringLiteral("Grid Status:"), m_gridStatusLine);
...
void AlarmWidget::updateGridStatus(const GridStatus &status)
{
    // Decode bit 0 of register 39065. Off-Grid means the inverter is operating
    // in EPS (Emergency Power Supply) / island mode: the public grid is
    // unavailable and the inverter is powering loads from PV + battery alone.
    if (status.offGrid) {
        m_gridStatusLine->setText(
            QStringLiteral("Off-Grid / EPS (0x%1)")
                .arg(status.raw, 4, 16, QLatin1Char('0')).toUpper());
        m_gridStatusLine->setStyleSheet(QStringLiteral("color: orange; font-weight: bold;"));
    } else {
        m_gridStatusLine->setText(
            QStringLiteral("On-Grid (0x%1)")
                .arg(status.raw, 4, 16, QLatin1Char('0')).toUpper());
        m_gridStatusLine->setStyleSheet(QStringLiteral("color: green; font-weight: bold;"));
    }
}
